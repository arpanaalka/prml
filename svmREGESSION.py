...
def findError(alpha,data,b,C,output,EPS):
	...
	for i in range(m):
		y2 = data[i][n-1]
		point = data[i,0:n-1]
		output[i] = svm_output(data,alpha,point,b)
		error[i] = output[i]-y2
...
def primal_object(alpha,data,EPS,b,C):
	out1 = 0
	out2 = 0
	m,n=data.shape
	for i in range(m):
...
def dual_object(alpha,data,EPS):
	out1 = 0
	out2 = 0
	m,n=data.shape
	for i in range(m):
...
def takeStep(i1,i2,alpha,data,b,C,error,output,EPS):
	...
	if eta < 0:
		...
		alpha1 = alpha
		alpha1[i2] = L
		Lobj = dual_object(alpha1,data,EPS)
		alpha1[i2]=H
		Hobj = dual_object(alpha1,data,EPS)
		if Lobj > Hobj+eps:
			a2 = L
		elif Lobj < Hobj-eps:
			a2 = H
...
def second_hueristic(alpha,data,b,c,error,i2,output,EPS):
	m,n=data.shape
	error = findError(alpha,data,b,C,output,EPS)
	E2 = error[i2]
	maxi = None
...
def check_example(i2,alpha,data,b,C,error,output,EPS,primal,dual):
	tol = 0.001
	m,n = data.shape
	error = findError(alpha,data,b,C,output,EPS)
	alph2 = alpha[i2]
	y2 = data[i2][n-1]
	E2 = error[i2]
	x = dual_object(alpha,data,EPS)
	y = primal_object(alpha,data,EPS,b,C)
	print x,y
	dual.append(x)
	primal.append(y)
	if (E2 > -EPS+tol and alph2 > 0) or (E2 < EPS-tol and alph2 < 0 ) or (E2 > tol and alph2==0) or (E2 < -tol and alph2==0):
		...
		k = len(indlist)
		if k!=0:
			i1 = second_hueristic(alpha,data,b,C,error,i2,output,EPS)
...
if __name__ == '__main__':
	...
	while num_changed > 0 or exam_all:
		...
		if exam_all:
			for i in range(m):
				num_changed += check_example(i,alpha,data,b,C,error,output,EPS,primal,dual)
		else:
			for i in range(m):
